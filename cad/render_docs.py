...
def main():
    ...
    shots = [
        ("hero.png",        asm,  dict(azim=-38, elev=16, w=1100, h=1500)),
        ("side.png",        asm,  dict(azim=-90, elev=4,  w=1000, h=1500, ortho=True)),
        ("exploded.png",    exp,  dict(azim=-38, elev=18, w=1100, h=1600)),
        ("base-bay.png",    base, dict(azim=-52, elev=62, w=1500, h=1200)),
        ("base-top.png",    base, dict(azim=-90, elev=89, w=1400, h=1400, ortho=True)),
        ("base-joint.png",  lidj, dict(azim=-44, elev=30, w=1400, h=1100)),
        ("bay-populated.png", bay, dict(azim=-54, elev=52, w=1500, h=1150)),
        ("bay-front.png",   bay,  dict(azim=-96, elev=16, w=1400, h=1000)),
        ("populated.png",   pop,  dict(azim=-38, elev=16, w=1100, h=1500)),
    ]
    for name, items, kw in shots:
        if not items:
            continue
        render(items, OUT / name, **kw)
